...
void view_ants(ant_struct ant, int tick){
    ...
    int y = 3;
    for(int i = 1; i <= total_ants; i++){
        mvwprintw(view_ants_wind, y, 1, "Ant Number: %d | Birth Tick: %d | Age: %d | POS: %d, %d", ant.ant_number.at(i), ant.ant_birth_tick.at(i), ant.ant_age.at(i), ant.posx.at(i), ant.posy.at(i));
        y = y + 1;
    }
    wrefresh(view_ants_wind);
